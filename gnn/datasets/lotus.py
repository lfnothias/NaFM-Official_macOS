...
class Lotus(InMemoryDataset):
    def __init__(self, root, transform=None, pre_transform=None):
        
        super(Lotus, self).__init__(root, transform, pre_transform)
    ...
    def process(self):
        for path, processed_path in zip(self.raw_paths, self.processed_paths):
            ...
            labels = pd.read_csv(path)["Labels"].tolist()
            samples = []
...
if __name__ == "__main__":
    dataset = Lotus(root="../../downstream_data/classification/Lotus")
